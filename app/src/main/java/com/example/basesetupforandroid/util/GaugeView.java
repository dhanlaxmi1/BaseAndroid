...
public class GaugeView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Adjust the start and sweep angles for semi-circle
        ...
        // Draw semi-circle arcs
        canvas.drawArc(rectfInner, startAngle, sweepAngleFirstChart, false, paintInnerArc1);
        canvas.drawArc(rectfInner, startAngle, sweepAngleSecondChart, false, paintInnerArc1);
        canvas.drawArc(rectfInner, startAngle, sweepAngleThirdChart, false, paintInnerArc1);

        // Draw needle
        Paint needlePaint = new Paint();
        needlePaint.setStyle(Paint.Style.STROKE);
        needlePaint.setStrokeWidth(strokePointerLineWidth);
        needlePaint.setColor(colorPointerLine);

        float centerX = x / 2;
        float centerY = y / 2;
        float radius = (constantMeasure - paddingInnerCircle) / 2;

        // Calculate start and end points of the needle
        float startX = centerX + (float) (radius * Math.cos(Math.toRadians(startAngle + rotateDegree)));
        float startY = centerY + (float) (radius * Math.sin(Math.toRadians(startAngle + rotateDegree)));
        float endX = centerX + (float) ((radius - mainCircleStroke) * Math.cos(Math.toRadians(startAngle + rotateDegree)));
        float endY = centerY + (float) ((radius - mainCircleStroke) * Math.sin(Math.toRadians(startAngle + rotateDegree)));
